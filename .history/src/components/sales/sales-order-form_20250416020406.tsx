...
// Define an interface for item type
...
export function SalesOrderForm({
  ...
  // Use the Zustand store for API operations
  const {
    createSalesOrder,
    updateSalesOrder,
  ...
  // Form default values - simplified like in item-form pattern
  ...
  // Initialize the form with minimal defaults
  const form = useForm<SalesOrderFormValues>({
    resolver: zodResolver(salesOrderSchema),
    defaultValues,
  });

  // Reset form when salesOrder changes - simplified
  useEffect(() => {
    if (salesOrder) {
      console.log("Setting form values from salesOrder");

      // Set form values directly rather than a full reset
      form.setValue(
        "customer",
        typeof salesOrder.customer === "object" && salesOrder.customer
          ? (salesOrder.customer as { _id: string })._id
          : salesOrder.customer || ""
      );
      form.setValue("reference", salesOrder.reference || "");
      form.setValue(
        "salesOrderDate",
        salesOrder.salesOrderDate || new Date().toISOString()
      );
      form.setValue(
        "expectedShipmentDate",
        salesOrder.expectedShipmentDate || ""
      );
      form.setValue("paymentTerms", salesOrder.paymentTerms || "Net 30");
      form.setValue("deliveryMethod", salesOrder.deliveryMethod || "");
      form.setValue("salesPerson", salesOrder.salesPerson || "");
      form.setValue(
        "discount",
        salesOrder.discount || { type: "percentage", value: 0 }
      );
      form.setValue("shippingCharges", salesOrder.shippingCharges || 0);
      form.setValue("adjustment", salesOrder.adjustment || 0);
      form.setValue("customerNotes", salesOrder.customerNotes || "");
      form.setValue("termsAndConditions", salesOrder.termsAndConditions || "");
      form.setValue("status", validStatus);
      form.setValue(
        "attachments",
        Array.isArray(salesOrder.attachments) ? salesOrder.attachments : []
      );
    }
  }, [salesOrder, form, validStatus]);
  ...
  // Add initial items if needed - simplified
  useEffect(() => {
    // Clear existing fields to prevent duplication
    if (fields.length > 0) {
      return; // Already have items
    }

    console.log("Initializing order items");
    ...
    if (
      ...
      console.log(
        `Adding ${salesOrder.items.length} existing items from sales order`
      );

      // Add items from salesOrder
      salesOrder.items.forEach((item: OrderItemType) => {
        if (!item) return;
  ...
  }, [salesOrder, fields.length, append]);
  ...
  useEffect(() => {
    // Only calculate if the form is initialized with actual values
  ...
  const calculateTotals = () => {
    ...
    try {
      // Ensure watchItems is an array before using reduce
      if (!watchItems || !Array.isArray(watchItems)) {
        setSubTotal(0);
        setTotal(0);
        return;
      ...
      if (watchDiscount) {
        if (watchDiscount.type === "percentage") {
          calculatedTotal -=
            (calculatedTotal * (watchDiscount.value || 0)) / 100;
        } else {
          calculatedTotal -= watchDiscount.value || 0;
        }
      }
  ...
  // Handle form submission - updated to use the Zustand store
  const onSubmit = async (data: SalesOrderFormValues) => {
    try {
      ...
      const salesOrderDate = data.salesOrderDate
        ? new Date(data.salesOrderDate).toISOString()
        : new Date().toISOString();

      const expectedShipmentDate = data.expectedShipmentDate
        ? new Date(data.expectedShipmentDate).toISOString()
        : undefined;
      ...
      if (salesOrder?._id) {
        await updateSalesOrder(
          salesOrder._id,
          orderData as unknown as SalesOrderFormValues
        );
        toast.success("Sales order updated successfully");
      } else {
        await createSalesOrder(orderData as unknown as SalesOrderFormValues);
  ...
  const isLoading = isStoreLoading || uploading;
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          onChange={(e) => {
                            if (e.target.value) {
                              // Convert the date to ISO format with time set to midnight
