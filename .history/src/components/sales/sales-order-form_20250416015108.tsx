...
export function SalesOrderForm({
  ...
  // Add a ref to track if we've reset the form for this sales order
  const resetTracker = useRef<string | null>(null);
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      salesOrderDate: salesOrder?.salesOrderDate
        ? new Date(salesOrder.salesOrderDate).toISOString()
        : new Date().toISOString(),
      expectedShipmentDate: salesOrder?.expectedShipmentDate
        ? new Date(salesOrder.expectedShipmentDate).toISOString()
        : "",
      ...
      status: validStatus,
      attachments: Array.isArray(salesOrder?.attachments)
        ? salesOrder.attachments
        : [],
    }),
    [salesOrder, validStatus]
  );
  ...
  // Reset the form when salesOrder changes or defaultValues change - with safeguards against infinite updates
  useEffect(() => {
    if (salesOrder && salesOrder._id) {
      // Only reset if we haven't already reset for this specific sales order
      if (resetTracker.current !== salesOrder._id) {
        console.log(`Form resetting with sales order ID: ${salesOrder._id}`);
        form.reset(defaultValues);
        // Track that we've reset for this sales order
        resetTracker.current = salesOrder._id;
      }
    }
  ...
  useEffect(() => {
    try {
      // We only want to append items if:
      // 1. fields array is empty (no items yet)
      // 2. We have valid sales order items data
      if (fields.length === 0) {
        console.log("Initializing items in field array");
        ...
        if (
          ...
          // (this is safe because we've already checked fields.length === 0)
          ...
          console.log("Adding single empty item");
          append({
            item: "",
            quantity: 1,
            rate: 0,
      ...
      // Add one empty item as fallback
      if (fields.length === 0) {
        append({
          item: "",
          quantity: 1,
  ...
  }, [append, fields.length, salesOrder?.items]);
