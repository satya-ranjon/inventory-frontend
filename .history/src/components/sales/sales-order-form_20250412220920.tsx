...
// Extended interface to include calculated fields not in the schema
interface ExtendedSalesOrderData extends Omit<SalesOrderFormValues, "items"> {
  subTotal?: number;
  total?: number;
  items: OrderItem[];
}
...
interface SalesOrderFormProps {
  salesOrder?: ExtendedSalesOrderData;
...
export function SalesOrderForm({
  ...
  // Form default values
  const defaultValues: Partial<ExtendedSalesOrderData> = {
    customer:
  ...
  // Initialize the form with the extended type
  const form = useForm<ExtendedSalesOrderData>({
  ...
  const calculateTotals = () => {
    ...
    try {
      ...
      // Calculate tax amounts if tax percentage is specified
  ...
  const onSubmit = async (data: ExtendedSalesOrderData) => {
    try {
      ...
      if (salesOrder?._id) {
        updateSalesOrder({
          id: salesOrder._id,
          // Cast back to SalesOrderFormValues
